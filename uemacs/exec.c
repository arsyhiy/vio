// По комментарию в программе можно понять, что файл exec.c содержит функции, 
// которые занимаются выполнением команд, обработкой командных строк, буферов,
// файлов и файлов запуска (startup files).

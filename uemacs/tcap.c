// Программа tcap.c реализует драйвер видео терминала для системы, использующей термкап (termcap), 
// который предоставляет абстракцию для взаимодействия с различными типами терминалов. Это довольно старый код,
// который поддерживает различные функции для манипуляции с текстом и экраном в терминале, такие как перемещение курсора, 
// очистка строк и экрана, использование реверсивного видео и многое другое

// This program implements file locking routines, which are modified by Petri Kutvonen. 
// The goal of these routines is to lock and unlock files to prevent simultaneous access 
// from multiple processes, ensuring that only one process can work with a file at a time.

// Эта программа, написанная на языке C, называется mkstr, и её основная цель — создать 
// файл строк с ошибками, обрабатывая исходные C-программы и извлекая строки, которые используются 
// в вызовах ошибки (error("...")).

// Этот код, в целом, выполняет множество операций для работы с текстом, типичных для
// текстовых редакторов командной строки, таких как vi, с поддержкой отмены изменений,
// перемещения и редактирования текста в реальном времени, включая макросы и операции с несколькими строками.
...
static char *sccsid = "@(#)ex_vops.c	1.3	(9.1	2/9/83)";
#include "ex.h"
#include "ex_tty.h"
#include "ex_vis.h"
	...
	register int cnt;
{
	...
	vcursat(cursor);
	doomed = i - cindent();
	vshowmode(value(TERSE) ? "r" : "REPLACE 1 CHAR");
	if (!vglobp) {
		c = getesc();
		if (c == 0) {
			vshowmode("");
